...
`DATABASE_URL` は localhost の場合、`.env`に記載するが、Vercel 等に host する場合は、Vercel の環境変数に設定する。

`schema.ts`は<GithubLink filePath="app/lib/db/schema.ts" />を使う。

正常に DB が設定できたら、以下のコマンドでスキーマを DB に反映する。
...
認証には [Better Auth](https://www.better-auth.com/) を使用している。
Better Auth 用のテーブル（`user`, `session`, `account`, `verification`）は<GithubLink filePath="app/lib/db/schema.ts" />に定義されており、上記の `drizzle-kit push` で自動的に作成される。

管理者ユーザーの登録は、dev サーバー起動後に Better Auth の API を使って行う。
...
curl -X POST http://localhost:3000/api/auth/sign-up/email \
  ...
  -d '{"username":"admin","password":"YOUR_PASSWORD","email":"admin@example.com","name":"admin"}'
```

`YOUR_PASSWORD` は適切なパスワードに置き換えること。

### 初期データの投入

Drizzle のシードスクリプトで初期データを投入する。
シードスクリプトは<GithubLink filePath="@robopo/web/app/lib/db/seed.ts" />

```shell
bun db:seed
```

上記を実行することで、以下のデータが投入される。

- 公式コース: THE一本橋（id=1）、センサーコース（id=2）
- テスト用コース: TestCourse、TestCourse2
- テスト用採点者: TestJudge（id=1）、審判B、審判C
- テスト大会: テスト大会、ロボサバ2026（開催中状態）
- テスト選手: 選手 A〜C（ゼッケン 001〜003）
- 大会割当: 全大会にコース・選手・採点者を割当済み（公式コースは全大会に自動関連づけ）

シード実行後、すぐに採点画面から大会を開始できる。
