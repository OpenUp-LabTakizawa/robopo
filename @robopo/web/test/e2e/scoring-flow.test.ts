...
test.describe("Scoring flow navigation", () => {
  test("redirects to home when umpireId is missing", async ({ page }) => {
    await page.goto("/challenge/1/1")
    // Should redirect to home because umpireId is missing
    await page.waitForURL("/")
    expect(page.url()).toContain("/")
  })

  test("navigating with umpireId shows player selection or error", async ({
    page,
  }) => {
    await page.goto("/challenge/1/1?umpireId=1")
...
test.describe("Scoring screen layout", () => {
  ...
  test("redirects to home when umpireId is missing on challenge page", async ({
    page,
  }) => {
