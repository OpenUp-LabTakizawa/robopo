...
export function SummaryTable({ id, courseList }: Props) {
  ...
  function handleSort(key: keyof CourseSummary) {
    ...
    function parseDateValue(
      ...
      if (!value || value === "-" || !isCompletedCourse(pointData, maxResult)) {
      ...
      function getVal(item: CourseSummary) {
        ...
        switch (key) {
          case "firstMaxAttemptTime":
            return parseDateValue(value, item.maxResult)
          case "playerFurigana":
            return typeof value === "string" ? value.toString() : ""
          case "playerBibNumber":
            return parseBibNumber(value)
          case "firstMaxAttemptCount":
            if (!isCompletedCourse(pointData, item.maxResult)) {
              return order === "asc" ? Infinity : -Infinity
            }
        ...
        <table className="table-pin-rows table-pin-cols table">
          <thead>
            <tr>
              ...
              {itemTitle("ゼッケン", "", "playerBibNumber")}
              {itemTitle("コース", "完走時刻", "firstMaxAttemptTime")}
              {itemTitle("完走は何回", "で達成?", "firstMaxAttemptCount")}
              {itemTitle("コースの", "最高得点", "maxResult")}
  ...
  return (
    <tr>
      ...
      <td className="border border-gray-400 p-2">
        {player.playerBibNumber ?? "-"}
      </td>
      <td className="border border-gray-400 p-2">
        {completed ? player.firstMaxAttemptTime : "-"}
      </td>
      <td className="border border-gray-400 p-2">
