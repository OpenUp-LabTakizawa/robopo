...
export async function assignById(req: NextRequest, mode: string) {
    ...
    try {
        ...
        const id = mode === "player" ? reqbody.playerId : mode === "course" ? reqbody.courseId : mode === "umpire" ? reqbody.umpireId : reqbody
        const competitionId = reqbody.competitionId
        if (!competitionId || !Array.isArray(id)) {
        ...
        for (const pid of id) {
            const existing = await db
                ...
                .where(
                    and(
                        eq(mode === "player" ? competitionPlayer.competitionId : mode === "course" ? competitionCourse.competitionId : competitionUmpire.competitionId, competitionId),
                        eq(mode === "player" ? competitionPlayer.playerId : mode === "course" ? competitionCourse.courseId : competitionUmpire.umpireId, pid)
                    )
                )

            if (existing.length === 0) {
                // 割り当てが無い場合、割り当てを追加
                await db.insert(mode === "player" ? competitionPlayer : mode === "course" ? competitionCourse : competitionUmpire).values({
...
export async function unassignById(req: NextRequest, mode: string) {
    ...
    try {
        ...
        for (const pid of id) {
            ...
            if (existing.length > 0) {
                // 割り当てがある場合、割り当てを解除
                await db.delete(mode === "player" ? competitionPlayer : mode === "course" ? competitionCourse : competitionUmpire).where(
                    and(
                        eq(mode === "player" ? competitionPlayer.competitionId : mode === "course" ? competitionCourse.competitionId : competitionUmpire.competitionId, competitionId),
                        eq(mode === "player" ? competitionPlayer.playerId : mode === "course" ? competitionCourse.courseId : competitionUmpire.umpireId, pid)
                    )
